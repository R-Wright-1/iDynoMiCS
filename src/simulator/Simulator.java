...
public class Simulator {
	...
	/**
	 * Boolean that notes that a file defining the agent conditions at the start
	...
	/**
	 * Where the parameter useAgentFile is set to true, this must be a parser of
	 * an XML file that describes the initial state in which the agents should
	 ...
	 * method employed in previous versions of iDynomics.
	 */
	...
	/**
	 * Array of Solver objects - one for each solver specified in the simulation
	 * protocol file.
	 */
	 ...
	 * 
	 * Creates the simulator object under which a simulation will be run for the
	 * conditions stated in the protocol file. This is called within the
	 * initSimulation method of the Idynomics class. The path to a protocol file
	 * and results folder is provided and the simulation initialised. Required
	 * species, solutes, world, and grids are created and set to their
	 * respective initial conditions, and result file objects created that will
	 * produce statistics at a specified frequency.
	 * 
	 * @param protocolFile
	 *            Path to the protocol file specifying the conditions under
	 *            which the simulation is run.
	 * @param resultPath
	 *            Path to a relevant directory where the simulation results will
	 *            be stored.
	 */
	public Simulator(String protocolFile, String resultPath) {
		try {
			 ...
			 * simulation in. If these have been specified, paths to these files
			 * will be stored in agentFile and bulkFile respectively.
			 */
			detectInputFiles(protocolFile);
			/*
			 * Read in the parameters in the SIMULATOR mark-up of the XML file.
			 * This call also deals with setting the time steps (adaptive or
			 * specified).
			 */
			createSimulator();
			/*
			...
			/*
			 * Set up the POV-ray writer. TODO Consider deleting as part of
			 * graphics overhaul.
			 */
	...
	/**
	 * \brief Method that starts the simulation, calling each timestep until the
	 * simulation time is over or the biofilm fills the domain.
	 */
	public void run() {
	...
	public void step() {
		try {
			 ...
			 * due to floating point issues). This will also output for sure on
			 * the last step.
			 */
			...
			if ((testTime >= 0.0) || SimTimer.simIsFinished()) {
				...
				// sonia 26.04.2010
				// only remove the agents from the system after recording all
				// the information about active
	 ...
	 * specifying whether this run is under chemostat conditions, the setting of
	 * the simulation timestep, and time interval between the generation of
	...
	public void createSimulator() {
		...
		/*
		 * Read the flag from protocol file to decide if this is a chemostat run
		 * (false by default)
		 * 
		...
		/*
		 * Invasion/Competition simulation - true if the simulation should stop
		 * once there is only one species left (set to false by default)
		 * 
		...
		if (localRoot.isParamGiven("agentTimeStep"))
			...
			// Set agentTimeStep to NaN and have it fixed later when used in
			// AgentContainer
		 ...
		 * this to remain at the default "onetime" rather than be set to null if
		...
		if (randomFile.exists()) {
			...
			try {
				...
				LogFile.writeError(e,
						"Simulator.createSimulator() while" + "trying to read in random number state file");
				System.exit(-1);
			} finally {
				try {
					...
					LogFile.writeError(e,
							"Simulator.createSimulator()" + " while trying to test random number generator");
					System.exit(-1);
				}
			}
		} else {
			...
			try {
				...
				LogFile.writeLogAlways("No valid random seed given in"
						+ "protocol file.\nUsing a randomly generated seed: " + randomSeed);
			}
			ExtraMath.random = new MTRandom(randomSeed);
			LogFile.writeLogAlways("Random number generator test: " + ExtraMath.random.nextInt());
		}
		/*
	 ...
	 * 
	 * @param resultPath
	 *            String stating the file path where these files should be
	 *            created
	 */
	public void createFiles(String resultPath) {
		...
		/*
		 * Initialise data files. We pass the current iterate to output files to
	...
	public void createDictionary() {
		...
		/*
		 * Build the dictionary of "particles". Use a trick to guarantee that
		 * the EPS compartment (capsule) is in last position (if it exists).
		 */
	 ...
	 * solutes involved in biofilm growth. The computationDomain mark-up defines
	 * the spatial region the biofilm will grow in. Only one world may be
	 ...
	 * regardless of the corresponding computationalDomain name. This method
	 * creates the world properties (e.g. system size and boundary conditions).
	 * In terms of boundary conditions, the current release allows you to create
	 * zero-flow, cyclic, constant and dilution boundaries. To create your own,
	 * create a new class extending the abstract class "BoundaryCondition".
	 */
	...
	public void recreateBulkConditions() throws Exception {
		...
		for (XMLParser aBulkRoot : simulationRoot.getChildrenParsers("bulk")) {
			...
			LogFile.writeLogAlways("\t\tInitializing bulk '" + bulkName + "' from input file.");
			...
			for (XMLParser aSoluteRoot : aBulkRoot.getChildrenParsers("solute")) {
				...
				LogFile.writeLog("\t\tsolute " + soluteName + " is now: " + soluteConcn);
			}
	...
	/**
	 * \brief Initialises each solute specified in the protocol file, creating a
	 * solute grid for each and setting the initial concentration
	 * 
	 * In iDynoMiCS each solute exists in its own spatial grid. This method
	 * iterates through each specified solute and creates the necessary grid
	 * space, in either 2D or 3D as specified in the protocol file. Once the
	 * grid is initialised, the grid is populated with values representing the
	 * initial concentration levels of the solute, taken from the Sbulk
	 * specification in the protocol file
	 * 
	 */
	...
	/**
	 * \brief Creates the species, agent grids, and populations required for
	 * this simulation.
	 * 
	 * This method is quite complex and is in three distinct parts. The first
	 * creates the species and progenitors, and registers these in the
	 * simulation. The second deals with the creation of the grids in which the
	 * species agents exist. Finally, populations of each species are created
	 * and initialised
	 * 
	 */
	public void createSpecies() throws Exception {
		...
		try {
			 ...
			 * Create a new species object for each specification and add to the
			 * list of species in this simulation.
			 */
		...
		try {
			...
			for (XMLParser spParser : _protocolFile.getChildrenParsers("species")) {
				...
				getSpecies(spParser.getName()).getProgenitor().initFromProtocolFile(this, spParser);
		...
		// NOTE WE ARE ONLY GOING TO DO STAGE 3 (AT THIS POINT IN DEVELOPMENT)
		// IF ONE TIME ATTACHMENT IS ON
		// FOR SELF ATTACHMENT, ENTRY OF AGENTS BEGINS WHEN THE SIMULATION
		// BEGINS

		if (this.attachmentMechanism.equals("onetime")) {
			System.out.print("\t Species populations: \n");
			/*
			 * If there is a previous state file, recreate the species that were
			 * present.
			 */
			if (useAgentFile)
				recreateSpecies();
	...
	/**
	 * \brief Create agents from a file describing individually all the agents
	 * species by species. Used if useAgentFile is true
	 * 
	 * It is possible to initialise the simulation with information in an agent
	 * file. If this is the case, the species will need to be created from
	 * information in this file. This method performs a simulation
	 * initialisation in that case
	 * 
	 * @throws Exception
	 *             Exception thrown if this file cannot be read correctly or
	 *             file is not consistent with protocol file
	 */
	public void recreateSpecies() throws Exception {
		...
		for (XMLParser aSpeciesRoot : simulationRoot.getChildrenParsers("species")) {
			...
			LogFile.writeLog(
					spName + " : " + speciesList.get(spIndex).getPopulation() + " agents created from input file.");
		}
	}

	/**
	 * \brief Introduces a new agent to the simulation, initialises the
	 * specified birthday and initial area parameters
	 * 
	...
	public void checkAgentBirth() {
		...
		for (XMLParser aSpRoot : _protocolFile.getChildrenParsers("species")) {
			 ...
			 * Thus the old code has been moved from here into its own method. A
			 * switch is used to determine which method of attachment is being
			 * employed. Although there are only 2 at the moment (in version
			 * 1.2), a switch is useful if further methods are added later.
			 * 
			 * Rob Apr 2015: Added the || to each update of creatingAgents. If
			 * the last species isn't creating agents but an earlier species is,
			 * the old method wouldn't have relaxed the grid.
			 */
			if (this.attachmentMechanism.equals("onetime")) {
				creatingAgents = this.oneTimeAttachmentAgentBirth(aSpRoot, spIndex, isCreatedByDivision) || creatingAgents;
			} else if (this.attachmentMechanism.equals("selfattach")) {
				creatingAgents = this.selfAttachmentAgentBirth(aSpRoot, spIndex, isCreatedByDivision) || creatingAgents;
			}
		}
		if (creatingAgents)
			agentGrid.relaxGrid();
	 ...
	 * the computation domain, where one time attachment is being employed
	 * 
	 * From Version 1.2 of iDynoMiCS, different methods of attachment are being
	 * introduced alongside the original one-time adhesion to the substratum.
	 * This method captures the case in the original version of the tool, where
	 * a number of cells are created at a random location within a set region of
	 * the domain.
	 * 
	 * @param aSpRoot
	 *            The XML tags contained within the "species" markup
	 * @param spIndex
	...
	/**
	 * \brief For self-attachment scenarios, creates the required number of
	 * agents per timestep. These migrate to the substratum or biofilm surface
	 * 
	 * For self attachment scenarios, cells are initialised on the boundary
	 * layer surface rather than the substratum. iDynoMiCS then models the
	 * agents move towards the biofilm surface using a run and tumble motion. If
	 * the cell moves towards the biofilm or surface, the move continues until
	 * the cell attaches. If the move takes the cell back towards the bulk, we
	 * assume the cell is back in the mix and introduce a further cell for a new
	 * attempt at attaching.
	 * 
	 * @param parser
	 *            The XML tags contained within the "species" markup
	 * @param spIndex
	 ...
	 * @return A boolean noting whether any agents have been created
	 */
	public boolean selfAttachmentAgentBirth(XMLParser parser, int spIndex, boolean isCreatedByDivision) {
		boolean creatingAgents = false;
		...
		if (SimTimer.getCurrentTime() >= speciesList.get(spIndex).cellInjectionStartHour
			...
			// We're in a cell injection period. In this time, we model cells
			// being injected into the domain. These may then
			...
			creatingAgents = create_Required_Number_Of_SelfAttaching_Agents(
					speciesList.get(spIndex).injectionOnAttachmentFrequency, parser, spIndex, isCreatedByDivision);
		} else {
			// Injection is off, attach at the different rate
			creatingAgents = create_Required_Number_Of_SelfAttaching_Agents(
					speciesList.get(spIndex).injectionOffAttachmentFrequency, parser, spIndex, isCreatedByDivision);
		}
	 ...
	 * injection period.
	 * 
	 * For self-attachment scenarios, it is possible to model injection of cells
	 * during the simulation, as well as periods that follow where no cells are
	 * injected. In these cases, the number of cells injected from the boundary
	 * layer will differ (to model the fact that the attachment rate may be
	 * different in these periods). This rate is supplied to this function, and
	 * the required number of cells created.
	 * 
	 * @param cellAttachmentFrequency
	 *            The number of cells to create at the boundary layer per hour,
	 *            for this injection period.
	 * @param parser
	 *            The XML object for this species. From the simulation protocol
	 *            file.
	 * @param spIndex
	 *            The index of this species in the simulation species list.
	 * @return A boolean noting whether any agents were created in this step.
	 */
	public boolean create_Required_Number_Of_SelfAttaching_Agents(Double cellAttachmentFrequency, XMLParser parser,
			int spIndex, boolean isCreatedByDivision) {
		/*
		 * To keep things consistent, the global timestep is specified in hours,
		 * and the number of cells injected is also specified per hour. Thus we
		 * need to work out we are injecting the correct number of cells here.
		 */
		...
		if (wholeAgentsThisTimeStep > 0) {
			speciesList.get(spIndex).createBoundaryLayerPop(parser, wholeAgentsThisTimeStep, isCreatedByDivision);
			return true;
		}
		return false;
	}
	...
	/**
	 * \brief Creates reports of concentrations of solutes and biomass on the
	 * default gird and a description of all agents
	 * 
	...
	public void writeReport() {
		...
		try {
			...
			for (Reaction aReac : reactionList) {
				...
				// KA - August 2013 - For each reaction, calculate the
				// production/uptake of each solute
				// Thus for each output period, we can report an estimate of
				// production/uptake of each solute
				// Where outputPeriod > simulation step, the output remains the
				// last simulation step. The user can interpolate
			...
			// KA AUGUST 2013
			// Now we're going to add to ENV_STATE the amount of solute produced
			// or consumed in this domain in this step
			summariseSoluteProductionOrUptake();

			// Add description of bulks
			// THIS COMPLETES THE ENV_STATE SOLUTE COUNTS, UNDER THE BULK NAME
			// TAGS
			...
			LogFile.writeError(e, "Simulator.writeReport() System description of agents");
		}
		...
		try {
			...
			LogFile.writeError(e, "Simulator.writeReport() System description of grids");
		}

	}

	/**
	 * \brief Calculates the production or uptake of each solute over all
	 * reactions for this timestep and writes them to the env_state file
	 * 
	 * Calculates the production or uptake of each solute over all reactions for
	 * this timestep and writes them to the env_state file. This gives some
	 * indication of the solute change in this simulation. Note that unlike
	 * other aspects of this simulation, the output here remains the solute
	 * figures from just the previous step should the output period be greater
	 * than the simulation timestep. The user will have to interpolate from
	 * there if this is the case
	 */
	public void summariseSoluteProductionOrUptake() {
		// KA AUGUST 2013
		// Now we're going to add to ENV_STATE the amount of solute produced or
		// consumed in this domain, in this step
		StringBuffer value = new StringBuffer();
		...
		for (int iSolute = 0; iSolute < soluteDic.size(); iSolute++) {
			...
			// Now we need the total production and uptake for this solute
			// across all the reactions.
	...
	/**
	 * \brief Deprecated internal chart creation method. Assumed deprecated as
	 * never called in code (KA 09/04/13)
	 * 
	 * Internal chart creation method. Assumed deprecated as never called in
	 * code (KA 09/04/13)
	 */
	...
	/**
	 * \brief Deprecated method. Not used in this version of iDynoMiCS (KA
	 * 09/04/13)
	 * 
	 ...
	 * 
	 * @param aSpeciesName
	 *            Text string noting the name of the species object required
	...
	/**
	 * \brief Return the integer reference specifying the position of a given
	 * solute in the solute dictionary
	 * 
	 * The createDictionary method created a dictionary of all solutes used in
	 * the simulation. This method is used to return the index of a particular
	 * solute, found by the given solute name
	 * 
	 * @param aSoluteName
	 *            The name of the solute whos position in the dictionary is
	 *            being sought
	 ...
	 * 
	 * @param aReactionName
	 *            The name of the reaction to return
	...
	/**
	 * \brief Returns an integer reference to the position of a solver in the
	 * simulation solver dictionary
	 * 
	 * @param aSolverName
	 *            The name of the solver to reference
	 * @return An integer value specifying the position of this solver in the
	 ...
	 * 
	 * @param aSolverName
	 *            Name of the solver to return from the array of solvers
	...
	/**
	 * \brief Determines whether a previous run is being restarted, reading in
	 * the respective last state if this is the case
	 * 
	 ...
	 * 
	 * @param protocolFile
	 *            The name of the protocol file for which the simulation is
	 *            being restarted
	 */
	public void detectInputFiles(String protocolFile) {
		...
		/*
		 * If we're restarting from a previous run, then we set the input files
		 * as the last files that were output.
		 */
